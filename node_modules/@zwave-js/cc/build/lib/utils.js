...
async function configureLifelineAssociations(applHost, endpoint) {
    ...
    if (lifelineGroups.length === 0) {
        ...
        if (endpoint.supportsCC(safe_1.CommandClasses["Association Group Information"])) {
            const agiAPI = API_1.CCAPI.create(safe_1.CommandClasses["Association Group Information"], applHost, endpoint);
...
must use node association:     ${rootMustUseNodeAssociation}`,
            ...
            if (!rootMustUseNodeAssociation) {
                if (isAssignedAsEndpointAssociation(node)) {
                    ...
                    const rootAssocAPI = API_1.CCAPI.create(safe_1.CommandClasses.Association, applHost, node);
...
async function assignLifelineIssueingCommand(applHost, endpoint, ccId, ccCommand) {
    const node = endpoint.getNodeUnsafe();
    if (node.supportsCC(safe_1.CommandClasses["Association Group Information"])
        ...
        if (groupsIssueingNotifications.length > 0) {
            ...
            const existingAssociations = getAssociations(applHost, node).get(groupId) ?? [];
            if (!existingAssociations.some((a) => a.nodeId === applHost.ownNodeId)) {
                applHost.controllerLog.logNode(node.id, {
                    endpoint: endpoint.index,
                    message: `Configuring associations to receive ${(0, safe_1.getCCName)(ccId)} commands...`,
                    direction: "outbound",
                });
