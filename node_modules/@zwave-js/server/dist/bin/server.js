...
(async () => {
    const args = (0, parse_args_1.parseArgs)([
        "_",
        "config",
        "mock-driver",
        "port",
    ...
    if (configPath && configPath.substring(0, 1) !== "/") {
        configPath = (0, path_1.resolve)(process.cwd(), configPath);
    }
    let options;
    let presetNames;
    if (configPath) {
    ...
    if (presetNames !== undefined) {
        ...
        presets = presetNames
            .map((name) => zwave_js_1.driverPresets[name])
            .filter((preset) => preset !== undefined);
    }
    const driver = args["mock-driver"]
        ? (0, mock_1.createMockDriver)()
        : new zwave_js_1.Driver(serialPort, options, ...(presets ?? []));
    driver.on("error", (e) => {
        console.error("Error in driver", e);
        // Driver_Failed cannot be recovered by zwave-js so we shut down
        if (e instanceof zwave_js_1.ZWaveError && e.code === zwave_js_1.ZWaveErrorCodes.Driver_Failed) {
            handleShutdown(1);
        }
    });
    let server;
    driver.once("driver ready", async () => {
        server = new server_1.ZwavejsServer(driver, {
            port: args.port,
            host: args.host,
